...
export default function ChatList({chats, setChats, selectedChatId, onSelectChat }: Props) {
  const [newChatName, setNewChatName]=useState<string>("");
  const [openNewChatPopup, setOpenNewChatPopup] = useState(false);
  const [search, setSearch] = useState('');


  useEffect(() => { (async () => setChats(await fetchChats()))() }, [setChats])
    ...
    <aside className="sidebar">
      <div className="sidebar-header">
        <div className="chatListHeader">
          ...
          >
            <button className="newChatButton" onClick={()=>setOpenNewChatPopup(true)}>
              New Chat
            </button>
          </Popup>
        </div>
        ...
        {filteredChats.map(c => (
          <li key={c.id}>
            ...
            >
              ...
              <div className="chat-time">{new Date(c.updated_at).toLocaleString()}</div>
            </button>
          </li>
        ))}
      </ul>
